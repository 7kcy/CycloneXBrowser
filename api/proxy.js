/**
 * CycloneX Proxy — api/proxy.js  (v2)
 *
 * Improvements over v1:
 *  - Robust URL validation & normalisation
 *  - Gzip / Brotli / Deflate decompression (via Node zlib)
 *  - Per-request 10 s timeout with AbortController
 *  - Attribute rewriting extended to: srcset, data-src, poster, background
 *  - Inline <style> block rewriting (not just style attributes)
 *  - Inline JS location pattern rewrites (onclick, etc.)
 *  - Smarter <base> injection (skips if one already present)
 *  - Full response-header scrubbing (CSP, HSTS, X-Frame-Options, COEP, COOP, …)
 *  - Rate-limit guard (100 req / 10 s per IP via in-process sliding window)
 *  - SSRF blocklist (localhost, RFC-1918, GCP metadata)
 *  - Cookie forwarding + parent notification
 *  - WebSocket best-effort pass-through
 *  - Detailed error page with retry button
 */
...
function proxyUrl(url, base) {
  if (!url) return null;
  const s = url.trim();
  if (/^(data:|javascript:|blob:|about:|#)/i.test(s)) return null;
  try {
    return PROXY_PATH + encodeURIComponent(new URL(s, base).href);
  } catch (_) { return null; }
}

// ── Attribute rewriter ────────────────────────────────────────────────────────
function rewriteAttr(html, tag, attr, base) {
  const re = new RegExp(`(<${tag}[^>]*?\\s${attr}=)(["'])([^"']*)(\\2)`, 'gi');
  return html.replace(re, (m, pre, q, url, qc) => {
    const p = proxyUrl(url, base);
    return p ? `${pre}${q}${p}${qc}` : m;
  });
}
...
function rewriteSrcset(html, base) {
  return html.replace(/(\ssrcset=)(["'])([^"']+)(\2)/gi, (_m, pre, q, val, qc) => {
    const rewritten = val.replace(/(https?:\/\/\S+|\/\S+)(\s+[\d.]+[wx])?/g, (part, url, desc) => {
...
function rewriteCssUrls(css, base) {
  return css
    .replace(/url\((['"]?)([^)"'\s]+)\1\)/g, (_m, q, url) => {
      const p = proxyUrl(url, base);
      return p ? `url('${p}')` : _m;
    })
    .replace(/@import\s+(['"])([^'"]+)\1/g, (_m, _q, url) => {
      const p = proxyUrl(url, base);
...
// ── Inline JS rewriter ────────────────────────────────────────────────────────
function rewriteInlineJs(js, base) {
  return js.replace(
    /(window\.location\.(?:href|assign|replace)\s*=\s*)(["'])([^"']+)\2/g,
    (_m, pre, q, url) => {
      const p = proxyUrl(url, base);
...
(function(){
  ...
  document.addEventListener('submit',function(e){
    ...
    if(method==='GET'){
      window.parent.postMessage({type:'cx-navigate',url:a.split('?')[0]+'?'+params},'*');
    } else {
      window.parent.postMessage({type:'cx-navigate',url:a,method:'POST',body:params.toString()},'*');
    }
  },true);
  // history
  ['pushState','replaceState'].forEach(function(fn){
    ...
    history[fn]=function(state,title,url){
      ...
      if(a){try{orig(state,title,PROXY+encodeURIComponent(a));}
...
body{margin:0;font-family:'Courier New',monospace;background:#080808;color:#ff6060;
     display:flex;align-items:center;justify-content:center;min-height:100vh}
.card{background:#111;border:1px solid #2a2a2a;border-radius:14px;padding:40px 44px;max-width:540px;width:92%}
h2{margin:0 0 20px;font-size:18px;color:#ff8585;letter-spacing:.03em}
.label{color:#555;font-size:10px;text-transform:uppercase;letter-spacing:.1em;margin-top:20px}
.code{background:#1a1a1a;border:1px solid #222;border-radius:7px;padding:12px 14px;
      margin-top:6px;color:#fc9;font-size:12px;word-break:break-all;line-height:1.6}
button{margin-top:24px;background:linear-gradient(135deg,#1e90ff,#0070d0);color:#fff;
       border:none;border-radius:9px;padding:11px 22px;cursor:pointer;font-size:13px;
       letter-spacing:.02em;transition:opacity .15s}
button:hover{opacity:.85}
.status{display:inline-block;background:#ff3030;color:#fff;border-radius:5px;
        padding:2px 9px;font-size:11px;margin-bottom:10px}
</style></head><body>
<div class="card">
  ...
  <div class="code">${safeTarget}</div>
  ${target ? `<button onclick="window.parent.postMessage({type:'cx-navigate',url:'${jsTarget}'},'*')">↩ Retry</button>` : ''}
</div></body></html>`;
}
...
module.exports = async function handler(req, res) {
  ...
  if (isRateLimited(ip)) {
    return res.status(429).send(errorPage('Too many requests — please slow down.', '', 429));
  }
  ...
  if (isBlockedHost(parsedUrl.hostname)) {
    return res.status(403).send(errorPage('Access to this host is blocked (private/reserved address).', targetUrl, 403));
  }
  ...
  try {
    ...
    for (const [k, v] of upstream.headers) {
      ...
      if (['etag','last-modified','cache-control','expires','content-length'].includes(kl)) {
        res.setHeader(k, v);
      }
    }
    ...
    if (rawCT.includes('text/html')) {
      ...
      // <form action>
      html = html.replace(/(<form[^>]*\saction=)(["'])([^"']+)(\2)/gi, (_m, pre, q, url, qc) => {
        const p = proxyUrl(url, base);
        return p ? `${pre}${q}${p}${qc}` : _m;
      });
      ...
      html = html.replace(/(\son\w+=)(["'])([^"']*)\2/gi,
        (_m, attr, q, js) => `${attr}${q}${rewriteInlineJs(js, base)}${q}`
      );

      // Strip CSP / X-Frame meta tags and referrer policy
      html = html
    ...
    // ── JavaScript ────────────────────────────────────────────────────────────
    if (rawCT.includes('javascript') || rawCT.includes('ecmascript')) {
      const raw = Buffer.from(await upstream.arrayBuffer());
      const js  = rewriteInlineJs((await decompress(raw, enc)).toString('utf8'), targetUrl);
      res.setHeader('Content-Type', rawCT);
    ...
    const msg     = timeout ? 'The target server did not respond within 10 seconds.' : err.message;
